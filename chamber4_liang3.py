...
if __name__ =="__main__":
    ...
    if(1):
        ...
        if (1):
            ...
            L[6],R[6],C[6]  =         2e-3, 0.21,93
            l6,r6,c6 =                0.015,0.143,1.13
        
        valve = [0,1,3,4] #Q node for valve
    
    if(1):##calculate elastance for atrium and ventricle
        HR = 60/T
        ...
        while(flag):
            ...
            for i in range(0,L.shape[0]):
                ...
                if i in valve and i!=1:
                    ...
                    if qj[i,0]>0:
                        # qj2[i,0] = (dp*dt+L[i]*qjm[i,0]) / (L[i]+R[i]*dt+B[i]*abs(qj[i,0])*dt)
                        qj2[i,0] = (dp*dt+L[i]*qjm[i,0]) / (L[i]+R[i]*dt)
                    else:
                        # qj2[i,0] = (dp*dt+L[i]*qjm[i,0]) / (L[i]+(R[i]+1000000)*dt+B[i]*abs(qj[i,0])*dt)
                        qj2[i,0] = (dp*dt+L[i]*qjm[i,0]) / (L[i]+(R[i]+1000000)*dt)
                else:
                    qj2[i,0] = (dp*dt+L[i]*qjm[i,0]) / (L[i]+R[i]*dt)
